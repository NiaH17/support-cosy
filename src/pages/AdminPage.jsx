        ...
        {isUser ? (
          <>
            ...
            {msg.message_fr && msg.message_en && (
              <button onClick={() => setShowFr(v => !v)} className="text-xs text-gray-400 mt-1 underline underline-offset-2">
          ...
          <>
            ...
            {msg.message_fr && (
              <>
                <button onClick={() => setShowFr(v => !v)} className="text-xs text-white/50 mt-1 underline underline-offset-2">
...
export default function AdminPage({ session }) {
  ...
  const [triage, setTriage] = useState({ part_needed: null, part_details: '', kraken_account: '', confirmed_issue: '' })
  ...
  async function saveTriage() {
    ...
    const { error } = await supabase.from('fr_support_requests').update({
            ...
            <div className="space-y-3">
              {filtered.map(req => (
                <button key={req.id} onClick={() => selectTicket(req)}
                  ...
                  <p className="font-semibold text-sm text-brand-navy">{ISSUE_LABELS[req.issue_type] || req.issue_type}{req.fault_code ? ` — ${req.fault_code}` : ''}</p>
    ...
    <div className="min-h-screen bg-gray-50 safe-top safe-bottom">
      ...
      <div className="bg-brand-navy px-6 pt-6 pb-4 sticky top-0 z-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold truncate">{selected.installation_company || selected.user_email}</p>
